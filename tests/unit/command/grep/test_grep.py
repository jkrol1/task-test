...
def mocked_line_match_grep(mocker: MockFixture) -> LineMatchGrep:
    ...
    input_processor.process.side_effect = [[ProcessingOutput(
...
def test_grep_execute(mocked_line_match_grep: LineMatchGrep, capsys: CaptureFixture[str]) -> None:
...
def test_suppress_binary_output_error_handling(mocked_line_match_grep: LineMatchGrep,
                                               capsys: CaptureFixture[str]) -> None:
    def _raise_suppress_binary_output_exception(_: ProcessingOutput, _2: Context) -> str:
        raise SuppressBinaryOutputError

    mocked_line_match_grep._create_output_message = _raise_suppress_binary_output_exception
    mocked_line_match_grep.execute()
